...
const PostModal = forwardRef<HTMLDivElement, PostModalProps>(
  ({ postNum, rerenderLikedSaved, rerenderPostData }: PostModalProps, ref) => {
    // 더보기 모달 Redux
    ...
    const [commentUserIdData, setCommentUserIdData] =
      useState<CommentUserIdData>({
        userId: userId, //해당 댓글의 USER_ID
        comId: null, //해당 댓글의 COM_ID
      });
    // CommentMore 컴포넌트에 전달할 데이터 가져오는 함수
    const getCommentUserIdData = (comId: number) => {
      setCommentUserIdData({
        userId: userId,
        comId: comId,
      });
    };
    const [postData, setPostData] = useState<PostData[]>([]); // 글 데이터
    const [commentData, setCommentData] = useState<CommentData[]>([]); // 댓글리스트 데이터
    const [userLikedSaved, setUserLikedSaved] = useState<UserLikedSaved[]>([]); //회원이 좋아요, 저장한 글 데이터
    ...
    const inputCommentChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const getTimeAgo = (post: PostData) => {
    ...
    return (
      <div className="postModal_popup">
          ...
          <div className="postModal" ref={ref}>
            <div className="postModal_image">
              <ImageSlider
                images={generateImagePaths(
                  brdId.brdId,
                  postData[0].BRD_IMAGE1,
                  postData[0].BRD_IMAGE2,
                  postData[0].BRD_IMAGE3,
                  postData[0].BRD_IMAGE4,
                  postData[0].BRD_IMAGE5
                )}
              />
            </div>
            <div className="postModal_content">
              <div className="postModal_header">
                <img
                  src={`http://localhost:4000/profileImg/${postData[0].USER_IMAGE}`}
                  alt="프로필 이미지"
                />
                ...
                {commentData &&
                  commentData.map((comment) => (
                    <div className="postModal_comment" key={comment.COM_ID}>
                      <img
                        src={`http://localhost:4000/profileImg/${comment.USER_IMAGE}`}
                        alt=""
                      />
              ...
              <div className="postModal_info">
                <div className="postModal_info_btns">
                  {userLikedSaved &&
                  userLikedSaved.some(
                    (element) => brdId.brdId === element.LIKED_NUM
                  ) ? (
                    <i
                      className="fa-solid fa-heart heart-style"
                      onClick={() => handleToggleLike(brdId.brdId)}
                    ></i>
                  ) : (
                    <i
                  ...
                  {userLikedSaved &&
                  userLikedSaved.some(
                    (element) => brdId.brdId === element.SAVED_NUM
                  ) ? (
                    <i
                      className="fa-solid fa-bookmark"
                      onClick={() => handleToggleSave(brdId.brdId)}
                    ></i>
                  ) : (
                    <i
                ...
                <div className="postModal_info_heart">
                  <span>
                    <strong>좋아요 {postData[0].LIKED_COUNT}개</strong>
                  </span>
                </div>
              ...
              <div className="postModal_comment_input">
                <input
                  ...
                  onChange={(event) => inputCommentChange(event)}
                  ref={commentInputRef}
                />
        ...
        {mainPostMoreVisible && (
          <PostMore
            value={{ postUserIdData }}
            rerenderPostList={rerenderPostList}
          />
        )}

        {mainCommentMoreVisible && (
          <CommentMore
